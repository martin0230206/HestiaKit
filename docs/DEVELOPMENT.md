...
- 新增工具時，路由與側欄不得各自手寫一份資料，必須透過 `src/tools/index.ts` 註冊。
...
- `src/tools/`：工具 metadata 的唯一來源，包含工具名稱、路徑、導覽標籤、圖示與頁面 component。
...
`name` 應穩定且唯一，供 router named route 與側欄使用。`path` 不應以 `/` 開頭，首頁工具使用空字串。
...
- `npm run test` 通過。
- `npm run build` 通過。
- 若修改 Dockerfile 或 Nginx 設定，`docker build -t hestiakit .` 通過。
- `dist/` 沒有被 staged。

...
本文件是 HestiaKit 後續開發與自動化代理協作的依據。新增功能、調整架構、整理檔案或由 AI agent 修改程式碼時，應優先符合這裡定義的分層、檔案結構與驗證規則。

## 代理協作規則

- 修改前先理解現有檔案與模式，不引入與既有架構不一致的新抽象。
- 不得覆蓋、還原或刪除使用者既有變更，除非使用者明確要求。
- 變更應保持聚焦，避免順手重構無關檔案。
- 若修改行為，回覆中應說明已驗證的命令；若無法驗證，也要明確說明原因。
- 優先使用 repo 既有工具、元件與 helper；只有在能降低實際複雜度或符合既有模式時才新增抽象。
- 新增套件前應確認必要性，並確保符合純前端與隱私優先定位。

## 專案定位

HestiaKit 是隱私優先的純前端瀏覽器工具箱。所有工具必須在使用者瀏覽器內完成處理，不應把使用者輸入、剪貼簿內容、上傳資料或產生結果送出裝置。
...
- 使用者資料預設不離開瀏覽器。
- 不得為工具核心功能新增遠端 API、遙測、追蹤或外部資料傳輸，除非使用者明確要求並理解影響。
...
- 使用 `localStorage`、clipboard、matchMedia 等瀏覽器 API 時，要處理不可用、權限拒絕或私密瀏覽情境。
- 本機轉換紀錄應限制數量，避免無界成長；既有工具多以 `hestiakit-*` 作為 storage key 前綴。
...
- 介面文字使用正體中文；技術縮寫或標準名稱可保留原文。
- 確保桌面與手機寬度下文字不重疊、不溢出主要控制項。
...
若因環境限制無法執行測試或建置，必須在回覆中明確說明未驗證項目與原因。
...
- `dist/` 沒有被 staged。
- `node_modules/`、log 與本機 IDE 設定沒有被 staged。
